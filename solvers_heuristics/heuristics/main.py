...
def sort_by_priority(jobs_list, mode=0):
    match mode:
        case 0:
...
while len(jobs) > 0:
    ...
    # print(all_current_machine_processing_times)
    ...
    for job_index, job in sort_by_priority(jobs, 1):
    ...
    current_time = min([job_on_machine[-1]['end_time'] for job_on_machine in job_order_on_machines]) + 1
    # t = min([job_on_machine[-1][]])
